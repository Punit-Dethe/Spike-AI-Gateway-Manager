...
export default function Hero() {
  return (
    ...
    >
      ...
      >
        Turn ChatGPT and Gemini into a{' '}
        <span className="relative inline-block">
          <span className="relative z-10">real REST API</span>
      ...
      >
        Spike runs an OpenAI-compatible gateway on your machine and exposes it publicly
        with one click. No API keys, no usage fees, just your browser session.
      </motion.p>
      ...
      >
        <Stat label="Models supported" value="13+" sub="GPT, o1, o3, Gemini" />
        <Stat label="Endpoint" value="OpenAI" sub="Drop-in compatible" />
        <Stat label="Public URL" value="One click" sub="No DNS, no account" />
        <Stat label="Cost" value="$0" sub="No API fees" />
      </motion.div>
    </section>
  );
}
...
function Stat({
  ...
  return (
    <div className="bg-sand-100 rounded-xl px-4 py-3 ring-1 ring-inset ring-sand-300/40">
      <div className="text-[11px] font-semibold tracking-wide uppercase text-gray-500">
        {label}
      </div>
      <div className="mt-1 text-2xl font-serif font-semibold text-gray-900">
        {value}
      </div>
